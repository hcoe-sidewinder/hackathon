...
const DonationDetail = ({ donation, onBack }) => {
  ...
  const trade = state.allTrades.find((trade) => trade._id == tradeId);
  // console.log(trade);
  // console.log(trade.doneeId.profilePic);
  // console.log(trade);
  console.log(trade);
  console.log(trade?.doneeId);
  console.log(trade?.doneeId.profilePic);
  // const doneeId = trade.doneeId;
  // console.log(doneeId);
  ...
  const handleVerifyOtp = async () => {
    if (otp === generatedOtp) {
      alert(
        `Payment of Rs. ${trade?.phaseId[0].amount} to ${trade?.nob} Successful! Waiting for BOQ verification.`
      );
      setShowOtpModal(false);
      try {
        console.log(`${import.meta.env.VITE_API_URL}trade/${tradeId}/pledge`);
        const response = await axios.put(
          `${import.meta.env.VITE_API_URL}trade/${tradeId}/pledge`,
          {},
          {
      ...
      <div className="bg-white/80 backdrop-blur-md shadow-xl rounded-md overflow-hidden w-full max-w-3xl p-8 transition-all duration-300 hover:shadow-2xl">
        ...
        <div className="flex items-center gap-6 border-b pb-6 group">
          ...
          <div className="transform transition-all duration-300 group-hover:translate-x-2">
            <h2 className="text-2xl font-bold text-textColor">{trade?.nob}</h2>
            <p className="text-gray-600 text-sm">PAN: {trade?.panNo}</p>
          </div>
        </div>
        ...
        <Section title="Payment Phases" icon={<Calendar className="w-6 h-6" />}>
          <div>
            {trade?.phaseId.length > 0 ? (
              <div className="space-y-2">
                {trade?.phaseId.map((phase, index) => (
                  <div
                    key={index}
      ...
      {showPaymentModal && (
        ...
        >
          <p className="text-gray-600 mb-4">
            You are pledging to <strong>{trade?.nob}</strong> for{" "}
            <strong>Rs. {trade?.phaseId[0].amount}</strong>.
          </p>
          <input
            type="password"
